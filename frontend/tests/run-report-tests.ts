...
import { runPhase14DReportsTests } from '../src/services/http/__tests__/reports.test';
...
try {
  ...
  const rOk = runPhase14DReportsTests();

  if (fOk && aOk && sOk && mOk && rOk) {
    console.log('\n✅ ALL PHASE 14A, 14B, 14C & 14D TEST SUITES: 100% PASS');
    process.exit(0);
  } else {
    console.error('\n❌ TEST SUITE FAILED');
    process.exit(1);
  ...
  console.error('\n❌ Error executing reporting test suite:', error);
  process.exit(1);
}
